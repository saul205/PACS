...
int main(int argc, char** argv)
{
  ...
  const int n_images = 20 * 2;
  //Replicar imagen
  // Crear vector con todas las imagenes
  unsigned char* images[n_images]; 
  ...
  fptr = fopen("times_lab6_mono.txt","a");
  ...
  for(int i = 0; i < n_images; i++){
    fprintf(fptr,"%.4f %.4f %.4f\n",wr_buffer[0][i] / 1000000.0, kernel_time[0][i] / 1000000.0, rd_buffer[0][i] / 1000000.0);
  }
  ...
  for(int i = 0; i < n_images; i++){
    ...
    if(i == n_images - 1){
      ...
      //image.display();
      snprintf(buffer, 60, "images/amd/%s/amd_%s_%s.bmp", argv[1], argv[1], argv[2]);
      image.save(buffer);
    }
